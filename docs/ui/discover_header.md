...
├─────────────────────────────────────────────┤
│ [ ] Surprise me worldwide                   │  4. Filters (Discover only)
...
| 4 | Filters | `_render_explore_filters()` | **Discover tab only** — Surprise me, Market, Sector |
...
| Active filter controls | Metric values and gloss |
...
- **Filter state in stats line** — scope is visible in filter widgets; overflow “Right now” line may summarize.
...
4. **About the data** expander — weekly refresh, five-metric gate, market breakdown
...
- [ ] Discover: filters + stats + top of card (or list) fit without horizontal scroll
