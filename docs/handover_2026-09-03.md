# Session handover -- Stock Swipe App (2026-09-03)

_Full point-in-time snapshot of `.claude/active_work.md`, archived when the live handover had
grown to ~146KB against its own stated 32,000-byte injection cap (the overflow was first flagged
by scope-auditor on 2026-08-28 at ~95KB and was never actioned before this pass). This is the
version committed on `main` as of 2026-09-03, i.e. it includes every MR through !88 (the handover
close-out for MR !87, the outlier-aware metric-range scaling change that closed out the last of
this session's nine Gemini-feedback backlog points) and the "## Status"/"## Current task"/
"## Infra" narrative going back through the GitHub-to-GitLab migration and the original
Sector/Lifecycle Router build. One correction made in the trim that follows, not in this archived
copy: this version's "## Step 3: sector-calibrated verdict thresholds" section frames sector/size
threshold calibration as "THE next fundamental piece" -- that proposal was explicitly considered
and DECLINED on 2026-09-01 (MR !79, Gemini feedback point 9); this archived copy predates that
decision and does not reflect it. This is historical record; for current state, read
`.claude/active_work.md`._
...
> Correction (see this file's own preamble): the section below frames sector/size threshold
> calibration as live and pending. It was explicitly considered and DECLINED (MR !79, Gemini
> feedback point 9) after this snapshot was taken. Historical record, left as originally
> written; do not act on it as current guidance.
