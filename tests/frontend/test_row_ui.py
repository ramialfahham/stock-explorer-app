...
def test_build_rich_row_html_includes_the_metric() -> None:
    html = build_rich_row_html(
        "Diageo", "DGE · Consumer Defensive", ("Return on equity", "31.4%")
    )
...
def test_build_rich_row_html_degrades_without_a_metric() -> None:
    ...
    renders title/subtitle, never a blank slot or an invented number."""
    html = build_rich_row_html("Diageo", "DGE · Consumer Defensive", None)
...
def test_build_rich_row_html_escapes_special_characters() -> None:
    html = build_rich_row_html(
        "<script>alert(1)</script>", "A & B", ("<b>label</b>", "1 & 2")
    )
    assert "<script>" not in html
    assert "&lt;script&gt;" in html
